...
function colormapColor(name: ColormapName, t: number): [number, number, number] {
  switch (name) {
    case "hsv":
      return hsvToRgb(t, 0.75, 1);
    case "cool":
      return hsvToRgb(0.5 + t * 0.33, 0.75, 1); // cyan → magenta
    case "warm":
      return hsvToRgb(t * 0.17, 0.75, 1); // red → yellow
    case "greyscale": {
      const v = 0.9 - t * 0.65;
      return [v, v, v];
    }
...
export function generateColormapColors(name: ColormapName, nshades: number, fontOpacity: number, bgOpacity: number): ColorEntry[] {
