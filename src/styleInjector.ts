...
function editorRules(level: number, font: string, bg: string, bold: boolean, italic: boolean): string {
  const textProps = [`color: ${font}`];
  if (bold) textProps.push("font-weight: bold");
  if (italic) textProps.push("font-style: italic");
  return [`.cm-line.HyperMD-header-${level} { background-color: ${bg}; }`, `.cm-line.HyperMD-header-${level} .cm-header-${level} { ${textProps.join("; ")}; }`].join("\n");
}

function readingRules(level: number, font: string, bg: string, bold: boolean, italic: boolean): string {
...
export function buildStylesheet(settings: HeaderColoringSettings): string {
  ...
  if (settings.mode === "colormap") {
    const colors = generateColormapColors(settings.colormapName, settings.nshades, settings.fontColorOpacity, settings.backgroundColorOpacity);
    ...
    if (settings.cycleColors) {
      ...
      LEVELS.forEach((level, i) => {
        ...
        if (!entry) return;
        if (settings.enableEditorMode) rules.push(editorRules(level, entry.font, entry.background, false, false));
        if (settings.enableReadingMode) rules.push(readingRules(level, entry.font, entry.background, false, false));
      });
    }
  } else {
    LEVELS.forEach((level) => {
      ...
      if (settings.enableEditorMode) rules.push(editorRules(level, font, bg, cfg.bold, cfg.italic));
      if (settings.enableReadingMode) rules.push(readingRules(level, font, bg, cfg.bold, cfg.italic));
    });
  }
